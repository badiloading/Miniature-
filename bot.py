...
async def run_ffmpeg(cmd):
    ...
    if process.returncode != 0:
        raise RuntimeError(stderr.decode() or "FFmpeg error")
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "🎬 مرحباً بك في بوت معالجة الفيديو\n\nاضغط 👇",
...
async def video_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not allowed:
        await update.message.reply_text("❌ الحد اليومي خلص")
        return
    ...
    try:
        ...
        cmd = [
            ...
            *inputs,
            "-i", LOGO_PATH,
            "-filter_complex",
            f"{vf}[base];[2:v]scale={LOGO_WIDTH}:-1[logo];"
            f"[base][logo]overlay=W-w-{LOGO_MARGIN}:{LOGO_MARGIN}",
            "-pix_fmt", "yuv420p",
            "-c:a", "copy",
            out
        ]
